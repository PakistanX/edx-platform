...
class CourseEnrolmentViewSet(viewsets.ModelViewSet):
    ...
    def enroll_users(self, request, *args, **kwargs):
        available_courses_count = CourseOverview.objects.filter(
            get_available_course_qs(), get_same_org_filter(self.request.user),
...
class AnalyticsStats(views.APIView):
    ...
    def get(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        data = {
            ...
            'course_in_progress': course_stats.get('pending') if course_stats.get('pending') is not None else 0,
            'completed_course_count': course_stats.get('completions',
                                                       0) if course_stats.get('completions') is not None else 0
        }
...
class CourseStatsListAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return CourseOverview.objects.filter(get_same_org_filter(self.request.user)).annotate(
